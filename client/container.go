...
func (container *container) NetOut(network string, port uint32, protocol api.Protocol) error {
	return container.connection.NetOut(container.handle, network, port, protocol)
}
